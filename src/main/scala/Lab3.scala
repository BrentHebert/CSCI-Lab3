object Lab3 extends jsy.util.JsyApplication {
   ...
   * Lincoln Samelson
   * 
   * Partner: Brent Hebert
   * Collaborators: Ali Hakimi
   * identikey: lisa8891
   */
    ...
    e match {
      ...
      case Call(e1, e2) => eToVal(e1) match {
        ...
        case Function(Some(func), varName, fBody) => 
          val v1 = Function(Some(func), varName, fBody)
          val f = extend(env, func, v1)
          eval(extend(f, varName, eToVal(e2)), fBody)
  ...
  def step(e: Expr): Expr = {
    e match {
      ...
      case Binary(Plus, e1, e2) => if (isValue(e1) && isValue(e2)) (e1, e2) match {
        ...
        else throw new AssertionError("Look at Binary Plus, probably an error.")
      ...
      case Binary(Minus, e1, e2) => 
        ...
        else throw new AssertionError("Look at Binary Minus, probably an error.")
        
      case Binary(Times, e1, e2) =>
        if (isValue(e1) && isValue(e2)) N(toNumber(e1) * toNumber(e2))
        else if (!isValue(e1)) Binary(Times, step(e1), e2)
        else if (!isValue(e2)) Binary(Times, e1, step(e2))
        else throw new AssertionError("Look at Binary Times, probably an error.")

      case Binary(Div, e1, e2) => 
        if (isValue(e1) && ( isValue(e2) && e2 != 0)) N(toNumber(e1) / toNumber(e2))
        else if (!isValue(e1)) Binary(Div, step(e1), e2)
        else if (!isValue(e2)) Binary(Div, e1, step(e2))
        else throw new AssertionError("Look at Binary Div, probably an error.")
